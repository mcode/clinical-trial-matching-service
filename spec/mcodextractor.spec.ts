...
describe('ExtractedMCODE Import', () => {
    ...
    it('checksCountOfExtractedProfiles', function () {
      ...
      expect(extractedData.getSecondaryCancerConditions().length).toBe(1);
      ...
      expect(extractedData.getCancerRelatedSurgicalProcedures().length).toBe(2);
    ...
    it('checkExtractedCancerRelatedSurgicalProcedure', function () {
      ...
      expect(extractedData.getCancerRelatedSurgicalProcedures().some((procedure) => procedure.reasonReference.reference_meta_profile == 'mcode-primary-cancer-condition')).toBeTrue();  
    });
